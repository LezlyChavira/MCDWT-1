...
$H^k$, and $LL^k$ as only $L^k$. More information at
...
\subsection{Scalability in MDWT}
...
quality). For example, subbands
$\{S_0.LL^2, S_1.LL^2, ..., S_{N-1}.LL^2\}$
% in
%
%\begin{verbatim}
%+---+---+-------+  +---+---+-------+     +---+---+-------+
...
%\end{verbatim}
%
represent the scale (number) 2 of the original video (the spatial
resolution of this is the resolution of $s$ divided by 4 in each
spatial dimension). To reconstruct the scale 1 (which is provided by the
...
The 2D-DWT incorporates to $S$ an interesting feature: usually, $H$
subbands has a lower entropy than $s$. This means that if we apply to
$S$ an entropy encoder, we can get a shorter representation of the
video than if we encode $s$ directly. This is a consequence of that
2D-DWT removes (most of) the spatial redudancy of the images of the
video (neighboring pixels tend to have similar values and their
substraction tends to produce zeros, which reduces the entropy and
...
\subsection{MCDWT forward butterfly}
...
\begin{figure}
\centering
\imgw{800}{graphics/forward_MCDWT.svg}
