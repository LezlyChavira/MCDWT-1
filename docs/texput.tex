...
\href{https://en.wikipedia.org/wiki/Video}{Video} data show high
amounts of redundancy, spatial and temporal. For this reason, most of
video encoders compress the input
\href{https://en.wikipedia.org/wiki/Pixel}{pixels} basically in two
stages: (1) a
\href{https://en.wikipedia.org/wiki/Decorrelation}{decorrelation}
stage in the \href{https://en.wikipedia.org/wiki/Time_domain}{time}
and in the
\href{https://www.quora.com/What-is-spatial-domain-in-image-processing}{spatial}
domains that produces a collection of spatially uncorrelated
\href{https://www.quora.com/What-is-spatial-domain-in-image-processing}{coefficients}, and (2) an entropy encoding phase to remove the statistical redundancy. These coefficients have two interesting features:
\begin{itemize}
\item Usually have a \textbf{smaller entropy} than the original pixels.
\item The transform domain provides a \textbf{multiresolution representation} of the visual information, or in other words, that the transform domain provides spatial \href{http://inst.eecs.berkeley.edu/~ee290t/sp04/lectures/videowavelet_UCB1-3.pdf}{scalability}.
\end{itemize}
...
(2D) decorrelation and, at the same time, a multiresolution
representation of such image. The forward transform transform the image into a set of subbands of different frequency with coefficients representing different spatial areas. 

\subsection{Wavelet and pyramid domains}
DWT domain is closely related to \href{https://en.wikipedia.org/wiki/Pyramid_(image_processing)}{Gaussian
  and Laplacian pyramids} because both (DWT and pyramids) are multiresolution, and it is quite fast to go from one domain to the other one and viceversa.
...
respectively.  More information at
...
Notice that the 1-iteration 2D-DWT can be applied $k$-times to the $LL$ subband
...
The DWT apllied to an image produces a scalable representation in
...
been transformed using the DWT of 2 levels will be represented by
...
DWT can be applied to a sequence of images simply transforming each image of the sequence independently. This is done, for example, in the
...
\subsection{Redundancy removed by MDWT}
...
to the use of DWT to each image of the sequence. All temporal redundancy remains in the encoded sequence.
...
transform. In a t+2D transform, the video is first analyzed over the
time domain and next, over the space domain. A 2D+t transform does
just the opposite.
...
\section{MCDWT}
We can achieve higher compression ratios
...
The DWT allows to get a scalable representation of a image, and
