...
\lstinputlisting[language=python]{../src/DWT.py}
...
\end{figure}
\lstinputlisting[language=python]{../src/MDWT.py}
...
\subsection{Redundancy and (previsible) compression}
...
\section{Removing temporal redundancy with MCDWT?}
...
\section{The MCDWT forward butterfly}
MCDWT is based on the idea that, to provide MC without transmitting
the motion information from the encoder to the decoder, the motion
information is computed at both ends, using only the information that
...
\end{figure}
\lstinputlisting[language=python,firstline=19,lastline=31]{../src/MCDWT.py}
